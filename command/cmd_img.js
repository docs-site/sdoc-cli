 ...
 */
async function processImagePaths(filePath) {
    ...
    return new Promise((resolve, reject) => {
        ...
        rl.on('line', (line) => {
             ...
             * - 回调函数检查路径是否需要添加'./'前缀
             */
            line = line.replace(/!\[.*?\]\((?!http)([^)]+)\)/g, (match, p1) => {
             ...
             * - 回调函数检查路径是否需要添加'./'前缀
             */
            line = line.replace(/<img\s+[^>]*src="(?!http)([^"]+)"[^>]*>/g, (match, p1) => {
                if (!p1.startsWith('./') && !p1.startsWith('http')) {
                    return match.replace(p1, `./${p1}`);  // 添加'./'前缀
                }
                return match;  // 如果已有前缀或是http路径则保持不变
 ...
 * @brief 导出图片路径处理相关功能
 * @namespace
 * @property {function} processImagePaths - 处理markdown文件中的图片路径
 * @param {string} filePath markdown文件路径
 * @return {Promise<void>} 无返回值
 */
module.exports = {
